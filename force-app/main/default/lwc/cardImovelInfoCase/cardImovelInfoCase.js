...
import SITUACAO from '@salesforce/schema/Case.IM_situacao_do_imovel__c';
...
export default class CardImovelInfoCase extends LightningElement {
    @api recordId;
    ...
    handleSuccess(event) {
        ...
        if (this.wiredCaseResult) {
            refreshApex(this.wiredCaseResult)
                .then(() => {
                    // troca pro view, AFTER refresh
                    this.mode = 'view';
                })
                .catch(() => {
                    // mesmo se refresh falhar, go to view
                    this.mode = 'view';
                });
        } else {
            this.mode = 'view';
